...
public class RemoveSymbolAction extends AbstractAction implements EventListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		int n = JOptionPane.showConfirmDialog(null, "Do you want to remove symbol '" + selectedSymbol.getTicker() + "' ?",
				"Removing symbol", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
		if (n == JOptionPane.YES_OPTION) {
			if (controller.remove(selectedSymbol)) {
				controller.setSelection((Symbol) null);
			} else {
				JOptionPane.showConfirmDialog(null, "The symbol '" + selectedSymbol.getTicker() + "' is currently used in transactions",
						CANNOT_REMOVE_SYMBOL, JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE);
			}
		}
	}
