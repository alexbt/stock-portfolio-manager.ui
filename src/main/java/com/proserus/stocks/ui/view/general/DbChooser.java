...
public class DbChooser implements EventListener, ActionListener {
	...
	private void init() {
		...
		JButton cancelButton = new JButton("Cancel (and shutdown)");
								...
								groupLayout
										...
										.addGroup(
												groupLayout
														...
														.addGroup(
																groupLayout.createSequentialGroup()
																		.addComponent(comboBox, 0, 300, Short.MAX_VALUE).addContainerGap())
														.addGroup(
		...
		if (SwingEvents.DATABASE_DETECTED.equals(event)) {
			...
			if (databases.getDatabases().size() == 1) {
				...
				for (String db : databases.getDatabases()) {
					i++;
					comboBox.setToolTipText(db);
					if (db.length() > 100) {
						db = THREE_DOTS + StringUtils.right(db, 100);
					}
					comboBox.addItem(i + COLON_SYMBOL + db);
				}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("ok")) {
			dialog.dispose();
			databases.setSelectedDatabase(comboBox.getSelectedItem().toString());
