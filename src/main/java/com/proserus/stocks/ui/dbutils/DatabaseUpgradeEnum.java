...
public enum DatabaseUpgradeEnum {
	...
	VERSION_21(21, new ExecuteScriptStrategy("21-1_createTables.sql"),//
			...
			// Do not delete, check if exist before creating... Use
			// SequenceNotExistStrategy
	...
	VERSION_24(24, new ExecuteScriptStrategy("24-1_defaultValues.sql"),//
			new ExecuteScriptStrategy("24-2_labelName.sql"),//
		...
		for (DatabaseUpgradeEnum e : values()) {
			for (DatabaseStrategy s : e.getStrategies()) {
				if (s instanceof ExecuteScriptStrategy) {
					Validate.notNull(DatabaseUpgradeEnum.class.getClassLoader()
							.getResourceAsStream(((ExecuteScriptStrategy) s).getScript()));
				}
			}
