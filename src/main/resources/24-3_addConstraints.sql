...
ALTER TABLE PUBLIC.HISTORICALPRICE ADD CONSTRAINT CK_HISTORICALPRICE_YEAR CHECK (YEAR >= 1800);
ALTER TABLE PUBLIC.HISTORICALPRICE ADD CONSTRAINT CK_HISTORICALPRICE_PRICE CHECK (PRICE >= 0);
ALTER TABLE PUBLIC.HISTORICALPRICE ADD CONSTRAINT CK_HISTORICALPRICE_CUSTOMPRICE CHECK (CUSTOMPRICE >= 0);
	
ALTER TABLE PUBLIC.LABEL ADD CONSTRAINT PK_LABEL_ID PRIMARY KEY (ID)
ALTER TABLE PUBLIC.LABEL ALTER COLUMN ID SET NOT NULL;
ALTER TABLE PUBLIC.LABEL ALTER COLUMN NAME SET NOT NULL;
ALTER TABLE PUBLIC.LABEL ADD CONSTRAINT UN_LABEL_NAME UNIQUE (NAME);
ALTER TABLE PUBLIC.LABEL ADD CONSTRAINT CK_LABEL_NAME CHECK (LTRIM(NAME) <> '');
...
ALTER TABLE PUBLIC.SYMBOL ADD CONSTRAINT CK_SYMBOL_TICKER CHECK (LTRIM(TICKER) <> '');
...
ALTER TABLE PUBLIC.TRANSACTION ADD CONSTRAINT CK_TRANSACTION_PRICE CHECK (PRICE > 0);
ALTER TABLE PUBLIC.TRANSACTION ADD CONSTRAINT CK_TRANSACTION_COMMISSION CHECK (COMMISSION >= 0);
ALTER TABLE PUBLIC.TRANSACTION ADD CONSTRAINT CK_TRANSACTION_QUANTITY CHECK (QUANTITY > 0);	
...
ALTER TABLE PUBLIC.VERSION ADD CONSTRAINT CK_VERSION_DATABASEVERSION CHECK (DATABASEVERSION >= 0);
